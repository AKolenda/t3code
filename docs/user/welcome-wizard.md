...
T3 Code finds directories that Claude Code, Codex, Cursor, or OpenCode has used.
Cursor and OpenCode must be enabled in Settings before importing their history.
Git repositories are listed first, newest activity on top. When the remote is
on GitHub, the
...
Imported projects include conversations from these agents active within the last
30 days. You can continue those conversations in T3 Code. Cursor imports include
locally cached desktop editor conversations and CLI history. Continuing a Cursor
import starts an agent session with the imported conversation as context; very
long conversations use the first prompt and recent history.
...
a time, ignores malformed records, and skips
unreadable or unparseable conversations.

Each import attempt reads up to 100 conversations within bounded history limits.
